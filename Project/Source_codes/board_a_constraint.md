...
**Constraints file for:** Board A (Primary)
...
| sw[0] | C9 | Add mode |
| sw[1] | B9 | Sub mode |
| sw[2] | G5 | Freeze (stops ACK, shows Err) |
| sw[3] | A7 | Shutdown (simulates power off) |
| sw[4] | C7 | Enable (must be active to allow transactions) |
...
| btn[8] | A5 |
| btn[9] | C6 |
| btn[10]–btn[15] | D4, F4, B6, B5, C4, E5 |

## LED Map (one-hot heartbeat running light)
| Port | Pin |
|------|-----|
| led[0] | D5 |
| led[1] | A3 |
...
|--------|-----|-----------|-------|
| pmod_ack_valid | M1 | Output | Continuous HIGH while Board A alive |
| pmod_bal_data | M2 | Output | Balance serial data A→B |
| pmod_bal_clk | P1 | Output | Balance serial clock A→B |
| pmod_hb_pos[0] | T2 | Input | PULLDOWN |
| pmod_hb_pos[1] | R3 | Input | PULLDOWN |
| pmod_hb_pos[2] | T3 | Input | PULLDOWN |
| pmod_hb_tick | T4 | Input | PULLDOWN |
| pmod_sync_data | R1 | Input | PULLDOWN |
| pmod_sync_clk | N1 | Input | PULLDOWN |

> PULLDOWN on all inputs keeps lines LOW when Board B is disconnected or powered off, preventing false signal detection.
...
set_property -dict {PACKAGE_PIN C7  IOSTANDARD LVCMOS33} [get_ports {sw[4]}]
...
set_property -dict {PACKAGE_PIN D4  IOSTANDARD LVCMOS33} [get_ports {btn[10]}]
set_property -dict {PACKAGE_PIN F4  IOSTANDARD LVCMOS33} [get_ports {btn[11]}]
set_property -dict {PACKAGE_PIN B6  IOSTANDARD LVCMOS33} [get_ports {btn[12]}]
set_property -dict {PACKAGE_PIN B5  IOSTANDARD LVCMOS33} [get_ports {btn[13]}]
set_property -dict {PACKAGE_PIN C4  IOSTANDARD LVCMOS33} [get_ports {btn[14]}]
set_property -dict {PACKAGE_PIN E5  IOSTANDARD LVCMOS33} [get_ports {btn[15]}]
...
set_property PULLDOWN true [get_ports {pmod_hb_pos[0]}]
set_property PULLDOWN true [get_ports {pmod_hb_pos[1]}]
set_property PULLDOWN true [get_ports {pmod_hb_pos[2]}]
set_property PULLDOWN true [get_ports pmod_hb_tick]
set_property PULLDOWN true [get_ports pmod_sync_data]
set_property PULLDOWN true [get_ports pmod_sync_clk]
```
